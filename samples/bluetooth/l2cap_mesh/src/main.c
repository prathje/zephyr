		...
		if (link != NULL) {
			...
			if (!bt_conn_get_info(link->conn, &info)) {
				if (info.role == BT_CONN_ROLE_MASTER) {
					int err = bt_l2cap_chan_connect(link->conn, &link->le_chan.chan, L2CAP_MESH_PSM);
					if (err) {
	...
	while(true) {
		...
		if (num_active_links > 0) {
			...
			if (link->chan_connected && atomic_get(&link->le_chan.tx.credits)) {
				...
				struct net_buf *buf = net_buf_alloc_len(&tx_data_pool, BT_L2CAP_CHAN_SEND_RESERVE+size, K_NO_WAIT);
